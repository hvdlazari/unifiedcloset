...
if($username) {
	...
	if($username == $USER->username) {
		$PAGE->navbar->add(get_string('unifiedcloset:my_closet', 'block_unifiedcloset'), new moodle_url('/blocks/unifiedcloset/view.php?username='.$username));
	}
	if($dirid) {
...
echo html_writer::empty_tag('link', array('href'=>"{$CFG->wwwroot}/theme/kroton/style/fonts.css",'rel'=>'stylesheet', 'type'=>'text/css'));
...
// echo html_writer::tag('script', null, array('src'=>"{$CFG->wwwroot}/lib/editor/atto/yui/src/editor/js/editor.js",'type'=>'text/javascript'));
...
// Langs
$PAGE->requires->string_for_js('unifiedcloset:validate:norecordsfound','block_unifiedcloset');
$PAGE->requires->string_for_js('unifiedcloset:form:find','block_unifiedcloset');
$PAGE->requires->string_for_js('unifiedcloset:form:mensagem_confirmacao','block_unifiedcloset');
$PAGE->requires->string_for_js('unifiedcloset:form:mensagem_confirmacao_dir','block_unifiedcloset');

//Header
// echo $OUTPUT->header();
//echo $OUTPUT->heading(get_string('unifiedcloset', 'block_unifiedcloset'));
